...
Clazz.load(["JU.Vibration", "JU.BS", "$.P3", "J.i18n.GT"], "J.modelkit.ModelKit", ["java.util.Arrays", "$.Hashtable", "JU.Lst", "$.M4", "$.Measure", "$.PT", "$.Quat", "$.SB", "$.V3", "JM.AtomCollection", "$.ModelSet", "JS.SV", "JU.BSUtil", "$.Edge", "$.Elements", "$.Escape", "$.Logger", "$.SimpleUnitCell", "JV.Viewer"], function(){
...
if (sym == null) sym = this.vwr.getSymTemp().setUnitCell( Clazz.newFloatArray(-1, [10, 10, 10, 90, 90, 90]), false, NaN);
...
var basis;
var sg = null;
var sgInfo = (noAtoms && !isDefined ? null : this.vwr.findSpaceGroup(isDefined ? null : bsAtoms, isDefined ? (isITA ? "ITA/" + name : name) : null, sym.getUnitCellParams(), false, true, false));
if (sgInfo == null) {
if (isITA) {
...
this.addOccupiedAtoms(i, bsSelected);
}
nAtoms = bsSelected.cardinality();
if (nAtoms == 1) {
return this.moveConstrained(iatom, bsFixed, bsModelAtoms, p, true, allowProjection);
}var p1 = JU.P3.newP(p);
p.x = NaN;
if (this.moveConstrained(iatom, bsFixed, bsModelAtoms, p1, false, true) != 1) {
return 0;
}var pd = JU.P3.newP(p1);
pd.sub(this.vwr.ms.at[iatom]);
var apos0 = this.saveAtomPositions();
...
p1.setT(this.vwr.ms.at[i]);
p1.add(pd);
if (this.moveConstrained(i, bsFixed, bsModelAtoms, p1, false, true) != 1) {
return 0;
...
p1.add(pd);
n += this.moveConstrained(ia, bsFixed, bsModelAtoms, p1, true, true);
}}
return (n > 0 && this.checkAtomPositions(apos0, apos, bsSelected) ? n : 0);
}, "JU.BS,JU.BS,JU.BS,~N,JU.P3,~A,~B");
Clazz.defineMethod(c$, "addOccupiedAtoms", 
function(iatom, bsOcc){
var a = this.vwr.ms.at[iatom];
if (this.vwr.ms.getOccupancyFloat(a.i) == 100) {
bsOcc.set(a.i);
} else {
this.vwr.ms.getAtomsWithin(0.0001, a, bsOcc, a.mi);
...
function(){
var pos =  new Array(this.vwr.ms.at.length);
...
;//5.0.1-v2 Mon Feb 05 11:35:56 CST 2024
