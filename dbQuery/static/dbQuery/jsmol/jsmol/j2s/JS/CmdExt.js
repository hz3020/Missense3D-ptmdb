...
Clazz.load(["JS.ScriptExt"], "JS.CmdExt", ["java.util.Hashtable", "JU.AU", "$.BS", "$.Base64", "$.Lst", "$.M3", "$.M4", "$.Measure", "$.P3", "$.P4", "$.PT", "$.Quat", "$.SB", "$.V3", "J.api.Interface", "J.atomdata.RadiusData", "J.c.STER", "$.VDW", "J.i18n.GT", "JM.AtomCollection", "$.LabelToken", "$.Measurement", "$.ModelSet", "$.Text", "JS.SV", "$.ScriptCompiler", "$.ScriptError", "$.ScriptEval", "$.ScriptInterruption", "$.ScriptMathProcessor", "$.T", "JU.BSUtil", "$.BoxInfo", "$.C", "$.Edge", "$.Elements", "$.Escape", "$.Logger", "$.Parser", "$.Point3fi", "$.SimpleUnitCell", "JV.FileManager", "$.StateManager", "$.Viewer"], function(){
...
var tokens = (asIdentity ? null : isAtoms ? JM.LabelToken.compile(this.vwr, label, '\0', null) : JM.LabelToken.compile(this.vwr, label, '\1', htValues));
...
if (asIdentity) str = modelSet.at[j].getInfo();
 else str = JM.LabelToken.formatLabelAtomArray(this.vwr, modelSet.at[j], tokens, '\0', indices, ptTemp);
} else {
var bond = modelSet.bo[j];
if (asIdentity) str = bond.getIdentity();
 else str = JM.LabelToken.formatLabelBond(this.vwr, bond, tokens, htValues, indices, ptTemp);
...
if (!this.chk) e.showString("" + this.vwr.findSpaceGroup(bs1, null, null, true, false, false));
return;
case 1086324752:
e.iToken = 1;
bs1 = (this.slen == 2 ? null : this.atomExpressionAt(2));
...
if (s.equals("rhombohedral")) s = "2/3a+1/3b+1/3c,-1/3a+1/3b+1/3c,-1/3a-2/3b+1/3c";
 else if (s.equals("trigonal") || s.equals("hexagonal")) s = "a-b,b-c,a+b+c";
ucname = s;
...
bs = bsModelAtoms;
} else if (mode == 1153433601 && this.tokAt(i) == 4 || mode == 1275069441) {
...
var w = (wyckoff.length > 1 || wyckoff.length == 0 ? '\0' : wyckoff.charAt(0));
if (w < 'a' && w != 'A' || w > 'z') this.invArg();
...
case 1275069441:
case 1086324754:
if (pt == null && bs == null && pts == null && wyckoff == null) this.invArg();
if (pts == null && pt != null) {
pts =  Clazz.newArray(-1, [pt]);
}var na = this.vwr.getModelkit(false).cmdAssignAddAtoms(type + (wyckoff != null ? ":" + wyckoff : ""), pts, bs, (isPacked ? "packed" : ""), this.e.fullCommand, isClick);
...
if (nm > 0) this.vwr.checkCoordinatesChanged();
if (this.e.doReport()) this.e.report(J.i18n.GT.i(J.i18n.GT.$("{0} atoms moved"), nm), false);
break;
case 134217764:
var s = this.vwr.assignSpaceGroup(bs, type);
