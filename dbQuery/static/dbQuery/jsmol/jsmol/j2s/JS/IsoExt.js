...
Clazz.load(["JS.ScriptExt"], "JS.IsoExt", ["JU.AU", "$.BS", "$.Lst", "$.M4", "$.Measure", "$.P3", "$.PT", "$.Quat", "$.SB", "$.V3", "J.api.Interface", "J.atomdata.RadiusData", "J.c.VDW", "JS.SV", "$.ScriptEval", "$.ScriptInterruption", "$.T", "JU.BSUtil", "$.BoxInfo", "$.C", "$.Escape", "$.Logger", "$.Parser", "$.TempArray", "$.Triangulator", "JV.JC"], function(){
...
i = eval.iToken;
}nth = ((!isSymop || target != null) && this.tokAt(i + 1) == 2 ? eval.getToken(++i).intValue : -1);
...
} else if (this.tokAt(i + 1) == 1073742327) {
nth = -2;
eval.iToken = ++i;
}if (xyz != null) {
i++;
...
if (s == null) s = this.vwr.getSymmetryInfo(iatom, xyz, iSym, trans, center, target, 135176, thisId, intScale / 100, nth, options, opList);
